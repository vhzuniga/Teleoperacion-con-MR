...
class TeleopBridgeNode(Node):
    ...
    def __init__(self):
        ...
        # ---------------- Gripper (nodo aparte: gripper_control/lite6_gripper_ws) ----------------
        self.cli_open_gripper = self.create_client(Trigger, '/gripper_open')
        self.cli_close_gripper = self.create_client(Trigger, '/gripper_close')
        self.cli_stop_gripper = self.create_client(Trigger, '/gripper_stop')

        # ---------------- Comandos de gripper desde Unity ----------------
        # Mensaje simple: std_msgs/String con valores "open", "close", "stop", "home"
    ...
    def robot_states_callback(self, msg: RobotMsg):
        ...
        # El robot está "listo para servo" solo si no hay error, está en modo 1
        # (ServoJ) y no está en un estado de STOPPED(4) o CONFIG_CHANGED(5).
        # Esto cubre tanto fallos (colisión) como cambios externos de modo
        # (por ejemplo, si activaste "modo manual" desde la página de UFactory).
        ready_now = (msg.err == 0) and (msg.mode == 1) and (msg.state not in (4, 5))

        if not self.initialized:
            if not ready_now:
                if not self.configuring:
                    self.get_logger().warn(
                        f'Robot no listo (err={msg.err}, mode={msg.mode}, state={msg.state}). Configurando...'
                    )
        ...
        if not ready_now:
            if not self.in_fault:
                self.get_logger().error(
                    f'¡Robot no listo para servo! err={msg.err} mode={msg.mode} state={msg.state}. '
                    'Pausando envío y reconfigurando...'
                )
            self.in_fault = True
            self.robot_ok = False
            if not self.configuring:
        ...
        # ready_now == True en este punto
        if self.in_fault:
            self.get_logger().info(f'Robot recuperado. Resincronizando con posición real: {real_angles}')
            self.last_commanded = real_angles
            self.in_fault = False
    ...
    # =========================================================
    #  CALLBACK: comandos de gripper desde Unity
    # =========================================================
    def gripper_callback(self, msg: String):
        ...
        if cmd == 'open':
            ...
            fut = self.cli_open_gripper.call_async(Trigger.Request())
            fut.add_done_callback(self._log_gripper_result('gripper_open'))
        elif cmd == 'close':
            self.get_logger().info('Gripper: CLOSE')
            fut = self.cli_close_gripper.call_async(Trigger.Request())
            fut.add_done_callback(self._log_gripper_result('gripper_close'))
        elif cmd == 'stop':
            self.get_logger().info('Gripper: STOP (apagar)')
            fut = self.cli_stop_gripper.call_async(Trigger.Request())
            fut.add_done_callback(self._log_gripper_result('gripper_stop'))
        else:
            self.get_logger().warn(f'Comando de gripper desconocido: "{cmd}"')

    def _log_gripper_result(self, label):
        def _cb(fut):
            try:
                res = fut.result()
                if not res.success:
                    self.get_logger().warn(f'{label} devolvió fallo: {res.message}')
                else:
                    self.get_logger().info(f'{label} OK: {res.message}')
            except Exception as e:
                self.get_logger().error(f'Fallo la llamada a {label}: {e}')
        return _cb
